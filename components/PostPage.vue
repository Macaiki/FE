<template>
  <div class="m-4 rounded-xl bg-neutral-800">
    <div class="block px-8 py-4">
      <div class="flex justify-between">
        ...
        <div class="text-white">
          <img :src="threads.imageURL" class="w-6 h-6 mr-2 rotate-90 lg:hidden"/>
        </div>
      </div>
    ...
    <div class="flex justify-between px-8 py-2 text-sm">
      <div class="flex justify-between">
        <div class="flex">
          <span class="flex px-8 py-2 border border-gray-600 rounded-l-lg cursor-pointer threadss-center hover:bg-gray-600" :class="[threads.isUpvoted !== 0 ? 'text-purple-600':'text-white', '']" @click="upvote">
            <svg class="w-4 h-4 mr-4" :class="[threads.isUpvoted !== 0 ? 'stroke-purple-600':'stroke-white', '']" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <span class="flex px-8 py-2 text-sm text-white border border-gray-600 rounded-r-lg cursor-pointer threadss-center hover:bg-gray-600" :class="[threads.isDownvoted !== 0 ? 'text-purple-600':'text-white', '']" @click="downvote">
            <svg class="w-4 h-4" :class="[threads.isDownvoted !== 0 ? 'stroke-purple-600':'stroke-white', '']" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="hidden px-4 rounded-lg cursor-pointer threadss-center lg:flex hover:bg-gray-700">
...
export default {
  ...
  mounted(){
    const date = new Date(this.threads.createdAt)
    this.waktu = new Intl.DateTimeFormat('id-ID', { dateStyle: 'long', timeStyle: 'medium' }).format(date)
    if(this.threads.createdAt == this.threads.updatedAt){
      this.edited = null
    }else{
      this.edited = "Edited"
    }
  ...
  // computed: {
  //   isUpvoted: function(){
  //     return this.threads.isUpvoted
  //   },
  //   isDownvoted: function(){
  //     return this.threads.isDownvoted
  //   }
  // },
  // methods:{
  //   upvote(){
